...
import { createOpportunityAcceptTool } from '../../../agent/tools/payment/opportunity-accept.tool.js';
import { createOpportunityRejectTool } from '../../../agent/tools/payment/opportunity-reject.tool.js';
...
import { ApprovalRepository } from '../../../database/repositories/approval.repository.js';
...
const approvalEngine = new ApprovalEngine(approvalRepository);
approvalEngine.registerNotifier(new EmailNotifier());
approvalEngine.registerNotifier(new WhatsAppNotifier());

export const uiToolRegistry = toolRegistry;
  ...
  try {
    ...
    if (payload && payload.toolName) {
      try {
        ...
        res.json({ success: true, message: 'Transaction authorized successfully', output: result.output });
      } catch (err: any) {
