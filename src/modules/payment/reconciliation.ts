...
import { OutboxEventRecord } from '../../infrastructure/database/repositories/outbox.repository.js';
...
export const createPaymentReconciliationHandler = (prisma: PrismaClient) => {
  return async (event: OutboxEventRecord): Promise<void> => {
    ...
    const buyerEmail = rawPayload?.payload?.payment?.entity?.notes?.buyerEmail || rawPayload?.payload?.order?.entity?.notes?.buyerEmail;
